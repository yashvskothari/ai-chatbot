...
from app.services.provider_service import analyze_image
from app.utils.docx_reader import extract_docx
from app.utils.image_reader import (
...
def process_file(
    ...
    if (
        ...
        and not is_supported_image(file.filename)
    ):
        raise ValueError(
            f"Unsupported file type '{extension}'. "
            "Supported types: PDF, DOCX, TXT, MD, PNG, JPG, JPEG, GIF, WEBP."
        )
    ...
    try:
        ...
        if is_supported_image(file.filename):
            ...
            try:
                ...
                analysis = (
                    f"(Image analysis unavailable: {exc})"
                )
        ...
        if not content.strip():
            ...
            raise ValueError(
                ...
                "It may be empty, scanned, or corrupted."
            )
